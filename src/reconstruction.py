...
def match_features(features, ratio_thresh=None):
    """Match SIFT descriptors between all pairs using Lowe's ratio test."""
    if ratio_thresh is None:
        ratio_thresh = data.LOWE_RATIO
...
def triangulate_matches(features, matches, poses, K):
    """Triangulate 3D points from matched features with known poses."""
    all_points = []
    all_colors = []
    ...
    for (idx1, idx2), match_list in tqdm(matches.items(), desc="  Triangulating"):
        ...
        P1 = data.build_projection(K, poses[idx1])
        P2 = data.build_projection(K, poses[idx2])
        ...
        for i, pt3d in enumerate(pts3d):
            ...
            if err1 < data.REPROJ_THRESH and err2 < data.REPROJ_THRESH:
...
def filter_point_cloud(points_3d, colors=None):
    """Clean up with voxel downsampling and statistical outlier removal."""
    ...
    pcd = pcd.voxel_down_sample(data.VOXEL_SIZE)
    pcd, _ = pcd.remove_statistical_outlier(
        nb_neighbors=data.OUTLIER_NEIGHBORS,
        std_ratio=data.OUTLIER_STD
    )
...
def build_sparse_reconstruction(images, poses, K):
    """Full pipeline: extract, match, triangulate, filter."""
